...
<body>
	...
	<section style="min-height: 350px;">
		<?php
			mysql_data_seek($result, 0) ;//reset result set 
			$row = mysqli_fetch_array($result);
			$query = ' select distinct(date_format(show_date,"%d %M")) as date,show_date,dayname(show_date) as day from showtimes where show_date>=curdate() and mid = '.$_GET['id'].' order by show_date limit 4;';
			mysqli_query($conn,$query) or die("Error querying database");
			...
			mysql_data_seek($result1, 0) ;//reset result set
			while($row1= mysqli_fetch_array($result1)){
