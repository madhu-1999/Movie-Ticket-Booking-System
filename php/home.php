...
<body>
    ...
    ?>
    <h1 style="margin: 20px;padding: 10px;">Movies</h1>

    <div class="card-wrapper">
        <aside class="aside" style="flex-basis:300px;">
            <?php
            $counter = 0;
        ...
        <section class="card-listing-wrapper">
            <?php
            $query = "SELECT * FROM (SELECT mid, mname, image, rating, GROUP_CONCAT(genre) AS genre, lang FROM movie_genre JOIN movies USING(mid) GROUP BY mid) AS x;";
            ...
            while ($row = mysqli_fetch_array($result)) {
                ...
                foreach ($genre_arr as $value) {
                    echo '<a href="search-misc.php?key=genre&val=' . $value . '"><pre style="color:#808080;font-size:12px;display: inline;">|' . $value . '</pre></a>';
                }
                echo '<a href="search-misc.php?key=lang&val=' . $row['lang'] . '"><pre style="color: #808080;font-size:12px;display: inline;">|' . $row['lang'] . '</pre></a></div><br/>';
            }
